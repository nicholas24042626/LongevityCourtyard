...
function render(n,depth=0,inLink=false){
 if(['679:499','483:1720','537:748','428:122','510:342','475:1390','688:979','plans-carousel'].includes(n.id)){
...
function header(slug){return `<header class="site-header"><div class="nav-canvas"><div class="brand"><a class="brand-home" href="index.html" aria-label="Longevity Courtyard home"><img src="assets/images/d3785a90c69bb02a1026498fd15e24ba8c36ef10.png" alt="Longevity Courtyard, in collaboration with HUR and Tzu Chi"></a><a class="tzu-chi-link" href="https://www.tzuchi.org.sg/en/" target="_blank" rel="noopener noreferrer" aria-label="Tzu Chi Singapore website" title="Tzu Chi Singapore"></a></div><button class="menu-toggle" aria-controls="main-navigation" aria-expanded="false" aria-label="Open navigation"><span></span><span></span><span></span></button><nav id="main-navigation" aria-label="Main navigation">${[['Programmes','programmes'],['Plans','plans'],['Fitness Team','fitness-team'],['Stories','stories'],['Equipment','resources']].map(([t,s])=>`<a href="${s}.html" ${slug===s?'aria-current="page"':''}>${t}</a>`).join('')}<div class="language-switch" aria-label="Language"><button type="button" lang="en" class="selected" aria-pressed="true">English</button><button type="button" lang="zh" aria-pressed="false">中文</button></div><a class="visit-button" href="visit.html">Visit Us!</a></nav></div></header>`;}
pages.push({slug:'plans',title:'Plans',root:{h:2200,children:[JSON.parse(JSON.stringify(pages.find(p=>p.slug==='index').root.children.find(n=>n.name==='Footer')))]}});
for(const page of pages){renderingPageSlug=page.slug;const {root}=page;const content=(page.slug==='plans'?fs.readFileSync(path.join(projectRoot,'src/plans.html'),'utf8').replace('<!-- plans-carousel -->',render({id:'plans-carousel',x:0,y:0,w:1440,h:403},1)):'')+root.children.map(n=>render(n,1)).join('\n');const html=`<!doctype html>
